                        ...
                        <div class="row">
                            <form action="{{ route('admin.categories.update', @$category->id) }}" method="post">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="panel panel-white">
                                        <div class="panel-heading">
                                            <h6 class="panel-title"><i class="icon-stack3 position-left"></i> Loại danh
                                                mục</h6>
                                        </div>
                                        <div class="panel-body">
                                            <div>
                                        ...
                                        <div class="panel-body">
                                            <div>
                                                <select id="selectIsActive" name="status"
                                                        class="bootstrap-select form-control select-lg">
                                                    <option
                                                        value="1" {{old('order', @$category->status) == 1 ? 'selected' : ''}}>
                                                        Công khai
                                                    </option>
                                                    <option
                                                        value="0" {{old('order', @$category->status) == 0 ? 'selected' : ''}}>
                                                        Ẩn
                                                    </option>
                                                </select>
                                            </div>
                                    ...
                                    <div class="panel panel-white">
                                        ...
                                        <div class="panel-body">
                                            <div class="list-locale">
                                                @foreach(Language::toSelectArray() as $keyLocale =>  $localeDes)
                                                    <div class="item">
                                                        @if(in_array($keyLocale, @$category->locales))
                                                            @if($category->language()->first()->language_code === $keyLocale)
                                                                <img class="icon-flag"
                                                                     src="{{ Language::getIconFlag($keyLocale)}}"
                                                                     alt="{{ $localeDes }}">{{ $localeDes }}
                                                                <span class="check"><i class="icon-check"></i></span>
                                                            @else
                                                                <a href="{{ route('admin.categories.edit', @$category->localeIds[$keyLocale]['reference_id']) }}"
                                                            ...
                                                            @endif
                                                        @else
                                                            <a href="{{ route('admin.categories.create', ['ref_language' => $keyLocale, 'from_id' => $category->id ]) }}"
